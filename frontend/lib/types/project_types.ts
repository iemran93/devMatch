...
export const createProjectSchema = z.object({
    ...
    project_type: z.array(z.number()).min(1, 'At least one project type is required'),
    technologies: z.array(z.number()).optional(),
    languages: z.array(z.number()).optional(),
    project_roles: z.array(z.object({
        title: z.string().min(1, 'Role title is required'),
        description: z.string().optional(),
        required_experience_level: z.number().min(1, 'Experience level must be between 1 and 5').max(5, 'Experience level must be between 1 and 5'),
